...
public class ViewFlow extends AdapterView< Adapter >
{
	...
	private void snapToScreen( int whichScreen )
	{
		...
		whichScreen = Math
				.max( 0, Math.min( whichScreen, getChildCount() - 1 ) );
		...
		mScroller
				.startScroll( getScrollX(), 0, delta, 0, Math.abs( delta ) * 2 );
		invalidate();
	}
	...
	public void setSelection( int position )
	{
		...
		for ( int offset = 1; mSideBuffer - offset >= 0; offset++ )
		{
			...
			if ( leftIndex >= 0 )
				mLoadedViews.addFirst( makeAndAddView(
						leftIndex,
						false,
						( recycleViews.isEmpty() ? null : recycleViews
								.remove( 0 ) ) ) );
			if ( rightIndex < mAdapter.getCount() )
				mLoadedViews.addLast( makeAndAddView(
						rightIndex,
						true,
						( recycleViews.isEmpty() ? null : recycleViews
								.remove( 0 ) ) ) );
